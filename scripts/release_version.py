...
def generate_release_notes(
    ...
    lines.extend([
        ...
        "Deploy PRAVAH to any Kubernetes cluster via our GitHub-hosted Helm repository or direct release archive:",
        "",
        "```bash",
        f"# Option 1: Install using GitHub URL directly from this release",
        f"helm upgrade --install pravah https://github.com/{repo_name}/releases/download/{tag_name}/pravah-{version}.tgz \\",
        f"  --namespace pravah --create-namespace \\",
        f"  -f custom-values.yaml",
        "",
        f"# Option 2: Add PRAVAH Helm repository via GitHub URL",
        f"helm repo add pravah https://raw.githubusercontent.com/{repo_name}/gh-pages/",
        f"# (or with GitHub Pages: https://{repo_name.split('/')[0]}.github.io/{repo_name.split('/')[1]}/)",
        f"helm repo update",
        f"helm upgrade --install pravah pravah/pravah \\",
        f"  --version {version} \\",
        f"  --namespace pravah --create-namespace \\",
        f"  -f custom-values.yaml",
        "```",
        ...
        f"- `pravah-v{version}.zip`: Standalone distribution archive containing `api/`, `web/`, and `scripts/` directories.",
        f"- `pravah-{version}.tgz`: Packaged Helm Chart archive.",
        "- `SHA256SUMS.txt`: Cryptographic integrity checksums for all release assets.",
        "",
    ])
